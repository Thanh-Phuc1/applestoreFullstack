...
controllerCart.addProductToCart = catchAsync(async (req, res, next) => {
  ...
  const LIMIT_PRODUCT_CAN_BUY = 50;
      ...
      products: [
        {
          ...
          imageUrl,
          quantity:
            quantity > 0 && quantity <= LIMIT_PRODUCT_CAN_BUY ? quantity : 1,
        },
      ],
      totalPrice: price,
    });
...
controllerCart.getListProductCart = catchAsync(async (req, res, next) => {
  ...
  if (!cart) {
    throw new AppError(
      404,
      "Do not find products in your cart ",
  ...
  return sendResponse(
    ...
    { cart },
    null,
    "Get list product in cart successful"
  );
});
